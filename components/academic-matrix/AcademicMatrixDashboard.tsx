...
export default function AcademicMatrixDashboard() {
  ...
  const exportSingleResource = (
    resource: ResourceMetric
  ) => {
    downloadCsv(
      `${resource.id}-resource.csv`,
      [
  ...
  const toggleResourceFullscreen = async (
    resourceId: string
  ) => {
    try {
      const element =
      ...
      console.error(
        'Unable to toggle resource fullscreen:',
        error
      )
    }
  }
