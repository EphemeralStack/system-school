...
function monthlyTrend(payments: Document[]): number[] {
  const now = new Date()
...
async function loadFinancialData(): Promise<FinancialData> {
  ...
  const ledger = payments.map((payment): LedgerRow => {
    ...
    return {
      ...
      studentName: `${text(user, ['FirstName'], 'Unknown')} ${text(
        user,
        ['LastName']
      )}`.trim(),
      form:
  ...
  return {
    ...
    monthlyTrend: monthlyTrend(payments),
  }
}
    ...
    <div className="flex h-16 items-end justify-center gap-2">
      {values.map((value, index) => (
        <span
          ...
          style={{
            height: `${value === 0 ? 3 : Math.max(8, (value / maximum) * 100)}%`,
          }}
          title={money(value)}
        />
      ))}
...
function KpiCard({
  ...
  return (
    <article>
      ...
      <div className={`${className} min-h-[145px] rounded-2xl p-4 text-white`}>
        {children}
        <p className="text-center text-2xl font-semibold">{value}</p>
        <p className="mt-2 text-center text-[9px] text-white/75">{caption}</p>
      </div>
    </article>
  )
}
  ...
  return (
    <div className="space-y-10 pb-10">
          ...
          >
            <div className="mb-2 flex h-16 items-end justify-center gap-2">
              <span
                ...
                style={{ height: `${Math.max(3, data.collectionRate)}%` }}
              />
              <span
          ...
          >
            <div className="mb-2 flex h-16 items-center justify-center">
              <div
                ...
                style={{
                  background: `conic-gradient(#111827 0 ${data.collectionRate}%, rgba(255,255,255,.85) ${data.collectionRate}% 100%)`,
                }}
              />
            </div>
          </KpiCard>
      ...
      <section>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-xs text-white"
          >
            ...
            <table className="w-full min-w-[720px] text-left text-xs">
              <thead className="bg-[#20283f] text-white">
                <tr>
                  ...
                  <th className="px-3 py-3">Fee ID</th>
                ...
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-gray-100">
                    ...
                    <td className="px-3 py-3">
                      ...
                      <p className="text-[10px] text-gray-400">{row.form}</p>
                    </td>
                    <td className="px-3 py-3">{row.feeId}</td>
                    ...
                    <td className="px-3 py-3">
                      <span
                        className={`rounded px-2 py-1 text-[9px] font-bold ${STATUS_STYLES[row.status]}`}
                      >
                        {row.status}
                      </span>
                    </td>
                    <td className="px-3 py-3">
                      <button
                        ...
                        className="flex items-center gap-1 rounded bg-blue-600 px-2 py-1 text-[9px] text-white"
                      >
      ...
      <section>
        ...
        <div className="grid gap-5 rounded-xl border border-gray-300 bg-white p-5 shadow-sm lg:grid-cols-[1.2fr_.8fr]">
          <div>
            <h3 className="text-sm font-bold">Recent payment activity</h3>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b px-5 py-4">
              <div>
                ...
                <p className="text-xs text-gray-500">{selected.feeId}</p>
              </div>
              <button
                type="button"
    ...
    <div className="pt-2">
      ...
      <div className="mt-6 border-t border-white/10 pt-5">
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-3 py-2.5 text-xs font-medium text-white"
        >
